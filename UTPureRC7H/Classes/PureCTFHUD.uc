...
simulated function xxFlagAddition(Canvas Canvas, CTFFlag Flag, int Y)
{
	...
	for (j = 0; j < 32; j++)
	{
		PRI = PlayerOwner.GameReplicationInfo.PRIArray[j];
		if (PRI != None && PRI.HasFlag == Flag)
		{
			Canvas.Font = MyFonts.GetSmallestFont(Canvas.ClipX);
			Canvas.StrLen(PRI.PlayerName, XL, YL);
			Canvas.SetPos(Canvas.ClipX - XL, Y + 64 * Scale);
			Canvas.DrawText(PRI.PlayerName);
			if (PRI.Team == PawnOwner.PlayerReplicationInfo.Team)
			{
				...
				Canvas.SetPos(Canvas.ClipX - XL, Y + 64 * Scale + YL);
				Canvas.DrawText(s);
			}
		}
	}	
